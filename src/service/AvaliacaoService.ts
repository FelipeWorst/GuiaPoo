...
export class AvaliacaoService {
    ...
    mapear(row: any): Avaliacao {
        return new Avaliacao(row.idAvaliacao, row.nota, row.comentario, row.data_avaliacao)
    }
    ...
    async criar(dados: Partial<Avaliacao>): Promise<Avaliacao | null> {
        ...
        if (!id_avaliacao || !nota || !comentario || !data_avaliacao) {
            throw new Error("Todos os campos são obrigatórios para criar um usuário.")
        }
        const query = `INSERT INTO guia.avaliacoes( id_avaliacao, nota, comentario, data_avaliacao ) VALUES ($1, $2, $3, $4) RETURNING *`
