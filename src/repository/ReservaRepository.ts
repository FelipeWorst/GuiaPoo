...
export class ReservaRepository implements InterfaceRepository<Reserva> {
    ...
    public mapear(row: any): Reserva {
        return new Reserva(row.id_reserva, row.data_reserva, row.data_volta, row.pago, row.email_usuario, row.email_guia, row.valor)
    }
    ...
    async inserir(dados: Reserva): Promise<Reserva> {
        ...
        const query = `INSERT INTO guia.Reserva(id_reserva, data_reserva, data_volta, pago, email_usuario, email_guia, valor) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`
        ...
        try {
            ...
            return new Reserva(rows[0].idReserva, rows[0].dataReserva, rows[0].dataVolta, rows[0].pago, rows[0].emailUsuario, rows[0].emailGuia, rows[0].valor)
        } catch (error) {
